...
function nav() { const items = session.role === 'artist' ? [['home','◈','工作台'],['works','▧','我的作品'],['open-calls','◉','展览征集'],['profile','○','个人资料']] : [['home','◈','工作台'],['library','▤','作品库'],['shows','◉','我的展览'],['profile','○','机构资料']]; return `<aside class="sidebar"><div class="brand">ART <em>LIAISON</em></div><nav class="nav">${items.map(x => `<button class="${page === x[0] ? 'active' : ''}" onclick="go('${x[0]}')"><i>${x[1]}</i>${x[2]}</button>`).join('')}</nav><div class="sidefoot">ART LIAISON<br>连接创作与展览<br><button onclick="logout()" style="margin-top:12px;color:#fff">退出登录 ↗</button></div></aside>`; }
function header(title, sub, action = '') { return `<div class="mainhead"><div><div class="eyebrow" style="color:var(--orange)">${session.role === 'artist' ? 'ARTIST STUDIO' : 'CURATOR DESK'}</div><h2>${esc(title)}</h2><div class="sub">${esc(sub)}</div></div><div class="user"><span>${esc(session.name)}</span><div class="avatar">${esc(session.name?.[0])}</div>${action}</div></div>`; }
function artCard(w) { return `<div class="art-card" style="--card:${w.color || '#b8c98a'};--accent:${w.accent || '#dc6c3c'}"><div class="tag">${esc(w.type || '绘画')} · ${esc(w.year || '—')}</div><h4>${esc(w.title)}</h4><p>${esc(w.material || '综合材料')} · ${esc(w.size || '—')}</p></div>`; }
function poster(s, allowApply = true) { return `<article class="poster" style="--poster:${s.poster || '#e85134'};--lime:${s.accent || '#cbff4d'}"><div><div class="date">OPEN CALL · 截止 ${esc(s.deadline || '待定')}</div><h3>${esc(s.title)}</h3></div><div><div class="meta">${esc(s.date || '时间待定')}<br>${esc(s.place || '地点待定')}<br>${esc(s.host || '')}</div>${allowApply ? `<button class="btn" style="margin-top:14px" onclick="applyShow('${s.id}')">投递作品 ↗</button>` : ''}</div></article>`; }
...
async function saveWork(event, id) { event.preventDefault(); const data = Object.fromEntries(new FormData(event.target)); const payload = {...data, artistId: session.id, artistName: session.name, color: colorSet[Math.floor(Math.random() * colorSet.length)], accent: '#dc6c3c', updatedAt: serverTimestamp()}; try { if (id) await updateDoc(doc(store, 'works', id), payload); else await addDoc(collection(store, 'works'), {...payload, createdAt: serverTimestamp()}); await loadData(); closeModal(); render(); toast('作品档案已保存'); } catch (error) { toast(friendlyError(error)); } }
function workModal(id) { const w = db.works.find(x => x.id === id) || {}; modal(id ? '编辑作品' : '添加作品', `<form class="form" onsubmit="saveWork(event,'${id || ''}')"><label>作品名称<input name="title" required value="${esc(w.title)}"></label><label>作品种类<select name="type"><option>${esc(w.type || '绘画')}</option><option>雕塑</option><option>影像</option><option>装置</option></select></label><label>材质<input name="material" required value="${esc(w.material)}" placeholder="如：油画、布面"></label><label>创作年份<input name="year" required value="${esc(w.year || '2026')}" placeholder="2026"></label><label>尺寸<input name="size" required value="${esc(w.size)}" placeholder="高 × 宽 cm"></label><label>风格（选填）<select name="style"><option>${esc(w.style || '—')}</option><option>抽象</option><option>具象</option><option>观念</option><option>极简</option></select></label><label>标价（选填）<input name="price" value="${esc(w.price)}" placeholder="人民币 ¥"></label><label>参加过的展览（选填）<input name="exhibitions" value="${esc(w.exhibitions)}"></label><label class="wide">作品简介<textarea name="description">${esc(w.description)}</textarea></label><label class="wide">作品图片链接（选填）<input name="image" value="${esc(w.image)}" placeholder="https://…"></label><div class="actions"><button type="button" class="btn ghost" onclick="closeModal()">取消</button><button class="btn lime">保存作品</button></div></form>`); }
...
Object.assign(window, { openAuth, auth, logout, go, workModal, saveWork, showModal, saveShow, applyShow, sendApp, viewApps, profileModal, saveProfile, filterWorks, closeModal });
