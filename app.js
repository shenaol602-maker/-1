...
function workRows(list) { return list.length ? list.map(w => `<tr><td><div class="tiny-art"><div class="swatch" style="--s:${w.color || '#dca265'}"></div><b>${esc(w.title)}</b></div></td><td>${esc(w.artistName || '匿名艺术家')}</td><td>${esc(w.material)}</td><td>${esc(w.year)}</td><td>${esc(w.size)}</td><td>${esc(w.style || '—')}</td><td><span class="status">${esc(w.exhibitionStatus || '可参展')}</span></td></tr>`).join('') : '<tr><td colspan="7" style="text-align:center;color:var(--muted);padding:40px">尚无艺术家作品。</td></tr>'; }
