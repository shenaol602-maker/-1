...
const materialChoices = ['\u6cb9\u753b', '\u4e19\u70ef', '\u6c34\u5f69', '\u6c34\u58a8', '\u7248\u753b', '\u7eb8\u672c', '\u6444\u5f71', '\u5f71\u50cf', '\u7efc\u5408\u6750\u6599', '\u9676\u74f7', '\u91d1\u5c5e', '\u6728\u6750', '\u7ea4\u7ef4', '\u6811\u8102', '\u73bb\u7483', '\u6570\u5b57\u5a92\u4ecb', '\u58f0\u97f3'];
const typeChoices = ['\u7ed8\u753b', '\u96d5\u5851', '\u6444\u5f71', '\u5f71\u50cf', '\u88c5\u7f6e', '\u7248\u753b', '\u6570\u5b57\u827a\u672f', '\u884c\u4e3a\u827a\u672f', '\u58f0\u97f3\u827a\u672f', '\u4e66\u6cd5', '\u9676\u827a', '\u8bbe\u8ba1', '\u7efc\u5408\u5a92\u4ecb'];
const yearChoices = Array.from({ length: new Date().getFullYear() - 1900 + 1 }, (_, i) => String(new Date().getFullYear() - i));
function optionList(id, values) { return `<datalist id="${id}">${[...new Set(values.filter(Boolean))].map(value => `<option value="${esc(value)}"></option>`).join('')}</datalist>`; }
...
function library() { const admin = session.role === 'admin'; const allMaterials = [...materialChoices, ...db.works.map(w => w.material)]; const allTypes = [...typeChoices, ...db.works.map(w => w.type)]; const allYears = [...yearChoices, ...db.works.map(w => w.year)]; return `${header(admin ? '作品管理' : '作品库',admin ? '管理员可删除全站作品。' : '使用条件筛选，在创作之间建立关联。')}<div class="filters"><input id="search" oninput="filterWorks()" placeholder="搜索作品名或艺术家…"><input id="mat" list="filter-materials" oninput="filterWorks()" placeholder="材质（可输入）"><input id="year" list="filter-years" oninput="filterWorks()" placeholder="年份（可输入）"><input id="type" list="filter-types" oninput="filterWorks()" placeholder="种类（可输入）">${optionList('filter-materials', allMaterials)}${optionList('filter-years', allYears)}${optionList('filter-types', allTypes)}</div><div class="tablewrap"><table><thead><tr><th>作品</th><th>艺术家</th><th>联系邮箱</th><th>材质</th><th>年份</th><th>尺寸</th><th>风格</th><th>状态</th>${admin ? '<th>操作</th>' : ''}</tr></thead><tbody id="workrows">${workRows(db.works)}</tbody></table></div>`; }
...
function workModal(id) { const w = db.works.find(x => x.id === id) || {}; const allMaterials = [...materialChoices, ...db.works.map(work => work.material)]; const allTypes = [...typeChoices, ...db.works.map(work => work.type)]; const allYears = [...yearChoices, ...db.works.map(work => work.year)]; modal(id ? '编辑作品' : '添加作品', `<form class="form" onsubmit="saveWork(event,'${id || ''}')"><label>作品名称<input name="title" required value="${esc(w.title)}"></label><label>作品种类<input name="type" list="artist-types" required value="${esc(w.type || '绘画')}" placeholder="可输入或选择"></label><label>材质<input name="material" list="artist-materials" required value="${esc(w.material)}" placeholder="可输入或选择"></label><label>创作年份<input name="year" list="artist-years" required value="${esc(w.year || '2026')}" placeholder="可输入或选择"></label>${optionList('artist-types', allTypes)}${optionList('artist-materials', allMaterials)}${optionList('artist-years', allYears)}<label>尺寸<input name="size" required value="${esc(w.size)}" placeholder="高 × 宽 cm"></label><label>风格（选填）<input name="style" value="${esc(w.style)}" placeholder="如：抽象、具象、观念"></label><label>参展状态<select name="exhibitionStatus"><option>${esc(w.exhibitionStatus || '可参展')}</option><option>正在展览中</option><option>不可参展</option><option>可商议</option></select></label><label>运费情况<select name="shipping"><option>${esc(w.shipping || '自付运费')}</option><option>不自负运费</option><option>无需运输</option><option>可商议</option></select></label><label>标价（选填）<input name="price" value="${esc(w.price)}" placeholder="人民币 ¥"></label><label>参加过的展览（选填）<input name="exhibitions" value="${esc(w.exhibitions)}"></label><label class="wide">作品简介<textarea name="description">${esc(w.description)}</textarea></label><label class="wide">上传作品图片<input name="imageFile" type="file" accept="image/*"><small>支持 JPG、PNG 等图片，单张不超过 10MB。</small></label><label class="wide">或填写作品图片链接（选填）<input name="image" value="${esc(w.image)}" placeholder="https://…"></label><div class="actions"><button type="button" class="btn ghost" onclick="closeModal()">取消</button><button class="btn lime">保存作品</button></div></form>`); }
...
function filterWorks() { const q = $('#search').value.toLowerCase(), f = ['mat','year','type'].map(id => $('#'+id).value.trim().toLowerCase()); $('#workrows').innerHTML = workRows(db.works.filter(w => (!q || `${w.title || ''}${w.artistName || ''}`.toLowerCase().includes(q)) && (!f[0] || String(w.material || '').toLowerCase().includes(f[0])) && (!f[1] || String(w.year || '').toLowerCase().includes(f[1])) && (!f[2] || String(w.type || '').toLowerCase().includes(f[2])))); }
...
setInterval(() => {
  if (session && page === 'open-calls') render();
}, 60 * 1000);
